...
run_name = 'nasari_100_rmswcbwexpws_w3-3'
#run_name = 'lexvec_doc2vec'
db_name = 'leefixsw'

def dump_dsctp_results():
    try:
        db_conn = sqlite3.connect('/home/fcmeng/workspace/data/%s.db' % db_name)
        db_cur = db_conn.cursor()
        db_cur.execute('select doc_id_pair, [{0}] from docs_sim'.format(run_name))
        outfile = open('/home/fcmeng/workspace/data/{0}_dump.txt'.format(db_name + '_' + run_name), 'w+')
