...
def cal_weight(arc):
    return math.exp(3.0 / (math.pow(arc, 3) + 1.0))


# calculate sim of 1 phrase vs 1 cluster (with lots of phrases)
def calculate_phrase_vs_cluster_sim(this_phrase, arc, c_phrases, word_pair_sims):
    ...
    weight = cal_weight(arc)
    this_p_c_sim_list = []
    for each_p in c_phrases:
        each_p = each_p.split('#')
        if len(each_p) == 1 and len(this_phrase) == 2:
            this_p_c_sim_list.append(weight*min(find_word_pair_sim(each_p[0], this_phrase[0]),
                                                find_word_pair_sim(each_p[0], this_phrase[1])))
        elif len(this_phrase) == 1 and len(each_p) == 2:
            this_p_c_sim_list.append(weight*min(find_word_pair_sim(each_p[0], this_phrase[0]),
                                                find_word_pair_sim(each_p[1], this_phrase[0])))
        elif len(each_p) == 2 and len(this_phrase) == 2:
            sim1 = min(find_word_pair_sim(each_p[0], this_phrase[0]), find_word_pair_sim(each_p[1], this_phrase[1]))
            sim2 = min(find_word_pair_sim(each_p[0], this_phrase[1]), find_word_pair_sim(each_p[1], this_phrase[0]))
            this_p_c_sim_list.append(weight*max(sim1, sim2))
...
def collect_all_phrases(target_doc_id, compare_sim_docs, json_files_path):
    ...
    for each_compare in compare_sim_docs:
        ...
        if target_doc_id[0] == each_compare[0].split('#')[0]:
            doc_key = 's1:'
            sent_idx_loc = 0
        elif target_doc_id[0] == each_compare[0].split('#')[1]:
            doc_key = 's2:'
            sent_idx_loc = 1
        else:
...
def build_single_doc_apv(phrase_cluster_by_clusterid, word_pair_sims, target_doc_id, compare_sim_docs, json_files_path, in_cur):
    ...
    for this_phrase, this_arc in all_phrases:
        for c_id, c_phrases in phrase_cluster_by_clusterid.items():
            if isinstance(apv_vec[c_id], list):
                apv_vec[c_id].append(calculate_phrase_vs_cluster_sim(this_phrase, this_arc, c_phrases, word_pair_sims))
            else:
                apv_vec[c_id] = [calculate_phrase_vs_cluster_sim(this_phrase, this_arc, c_phrases, word_pair_sims)]
