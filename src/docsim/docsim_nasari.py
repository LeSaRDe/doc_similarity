...
def write_intermedia_to_file(doc1_id, doc2_id, json_data):
    fname = "/home/fcmeng/workspace/doc_pair_sim/%s#%s.json" % (doc1_id, doc2_id)
...
def sim_from_tree_pair_graph(inter_edges, graph, tree_1, tree_2):
    ...
    return sum(cycle_weights), min_cycle_basis
...
def sent_pair_sim(sent_treestr_1, sent_treestr_2):
    ...
    sim, min_cycle_basis = sim_from_tree_pair_graph(inter_edges, tp_graph, tree_1, tree_2)
    # pid = os.getpid()
...
def doc_pair_sim(doc1, doc2):
    ...
    out_json = {'sim':0.0, 'cycles':[]}
    for sent_treestr_1 in l_sent_treestr_1:
        for sent_treestr_2 in l_sent_treestr_2:
            #TODO:
            sim_res, min_cycle_basis = sent_pair_sim(sent_treestr_1, sent_treestr_2)
            if sim_res != 0:
                out_json['sim'] += sim_res
                out_json['cycles'].append(min_cycle_basis)
...
def text_sim(db_cur):
    db_cur.execute('SELECT doc_id, parse_trees FROM docs WHERE parse_trees is NOT null')
    ...
    for i, doc1 in rows:
        for j, doc2 in rows:
            if i<j:
                valid_doc1, valid_doc2 = validate_doc_trees(doc1, doc2)
                p = multiprocessing.Process(target=doc_pair_sim, args=((i,valid_doc1), (j,valid_doc2)))
                proc_id += 1
                sim_procs.append(p)
                p.start()
                if proc_id >= 5000:
